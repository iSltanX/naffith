...
describe('الكتابة', () => {
   ...
   *
   * `saveSettings` تكتب `schemaVersion` كما وصلها، فمستدعٍ يبني الإعداد بيده
   * (أو يمرّر قيمةً نجت من نسخة أقدم) يكتب إصدارًا لا يقبله `loadSettings`،
   * فتضيع القيمة صامتةً في التشغيل التالي بوصفها «تالفة». الوعد المعلَن أن
   * كل قيمة مكتوبة تحمل إصدار البناء الحالي، والموضع الوحيد القادر على ضمانه
   * هو الكتابة نفسها:
   *
   *     JSON.stringify({ ...settings, schemaVersion: SETTINGS_SCHEMA_VERSION })
   */
  it.fails('تبصم الإصدار الحالي مهما كان الإصدار الممرَّر', () => {
    const store = fakeStorage();
