...
describe('ترتيب الاستعلامات لا يقرّر شيئًا', () => {
  ...
  it('الإيقاع منحنياتٌ تقرأ العرض وحده، لا درجاتٍ في استعلامات', () => {
    // الأربعة معلَنةٌ مرّةً واحدة، في `.onboarding` وحدها، وكلٌّ منها `clamp()`
    // يقرأ `vw`. `clamp(a, k·vw − c, b)` غير متناقصة في العرض بحكم تعريفها،
    // فلا يمكن لنافذةٍ أضيق أن تُنتج مقدارًا أكبر.
    //
    // وكان الإيقاع يقرأ `vh`، فكان ارتفاع البطاقة يتبع ارتفاع النافذة: كل بكسل
    // يُنقص من النافذة يُنقص ‎0.44 من البطاقة، فلا يُستردّ من الفراغ إلا ‎0.56 —
    // ولذلك كان الإطار لا يحيط بالبطاقة مهما قصرت النافذة. والمحور الصحيح هو
    // العرض: هو ما يقرّر التفاف النصّ، أي ما يقرّر ارتفاع البطاقة فعلًا.
    for (const name of ['--v-pane', '--v-block', '--v-tight', '--v-gap']) {
      ...
      expect(value, `${name} يقرأ الارتفاع`).not.toMatch(/vh/);
    }
    // ولا يبقى في الورقة مقدارٌ يقرأ الارتفاع، عدا حدَّي الإطار على المرأى
    // (`min/max-height: 100vh`) — وهما ليسا إيقاعًا: الأول يبسط السطح المقلوب
    // على النافذة كلها، والثاني يحبس التمرير داخل الشاشة بدل أن يهاجر إلى
    // المستند. الإحاطة تأتي من ارتفاع النافذة في `tauri.conf.json`، لا من
    // تصغير الإطار — ولو قرأ الإيقاع الارتفاع لعاد الاقتران الذي منعها.
    const withoutViewportBounds = BARE.replace(/(min|max)-height:\s*100vh;/g, '');
    expect(withoutViewportBounds, 'بقي مقدارٌ يقرأ الارتفاع في الورقة').not.toMatch(/\d\s*vh\b/);
  });
});
